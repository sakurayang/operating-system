...
fn handle_process_input() -> Result<Memory, u8> {
    println!("请输入{}，输入 r 释放内存", String::from("进程大小").blue().bold());

    let input = get_input();
    // 因为没有丢表进来，又不想丢，干脆返回一个特殊的数字，让后面来判断
    if input == "r" { return Err(8); }

    let size = input.parse::<Size>().unwrap_or(0);
    if size > SIZE { return Err(1) }
    if size <= 0 { return Err(2) }
...
fn handle_free_choose() -> Result<(), ()> {
    ...
    let input = get_input();
    return if input.is_empty() || input.to_lowercase() == "y" { Ok(()) } else { Err(()) }
}

fn handle_free(memory: &mut MemoryTable) {
    return loop {
        display_memory_table(memory.clone(), SIZE);
        println!("请选择{}，输入 q 退出", String::from("需要释放进程的编号").blue().bold());
