...
fn job_input() -> Vec<Job> {
    ...
    println!(
        "请按顺序输入{}，空格分隔，按 o 确定，按 c 取消",
        text.with(Color::Blue).attribute(Attribute::Bold)
    );
    loop {
        let mut input = String::new();
        stdin().read_line(&mut input).expect("Failed");
...
fn main() {
    ...
    for job in run_job {
        ...
        for value in values {
            value_row.add_cell(
                Cell::new(&*value.to_string()).with_style(Attr::ForegroundColor(color::WHITE)),
            )
        }
        value_row.add_cell(
            Cell::new(&*job.id.to_string()).with_style(Attr::ForegroundColor(color::WHITE)),
        );
    ...
    let avg_turnaround_text =
        format!("{}{:.3}", "平均周转时间为：".green(), avg_turnaround).underlined();
    let avg_weighted_text = format!(
        "{}{:.3}",
        "平均带权周转时间为：".green(),
